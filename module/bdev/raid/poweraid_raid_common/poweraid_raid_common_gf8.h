 ...
 *
 *   实现分层:
 *     - poweraid_raid_common_gf8.c:     软件参考实现（查表，正确性基准）
 *     - poweraid_raid_common_gf8_x86.c: x86 SSE/AVX2（pshufb 16 路并行 + non-temporal store）
 *     - poweraid_raid_common_gf8_neon.c: ARM NEON（vqtbl1q 16 路并行 + stnp non-temporal）
 *
...
#ifndef POWERAID_RAID_COMMON_GF8_H
#define POWERAID_RAID_COMMON_GF8_H
 ...
 * 线程安全，仅需调用一次（模块加载时）。*/
int  poweraid_raid_common_gf8_init(void);

/* 模块清理 */
void poweraid_raid_common_gf8_cleanup(void);
 ...
 *
 * 返回: 0 成功, 负数错误码 */
int poweraid_raid_common_gf8_encode(uint32_t data_chunks,
 ...
 *
 * 返回: 0 成功, 负数错误码 */
int poweraid_raid_common_gf8_decode_2(uint32_t data_chunks,
...
extern uint8_t poweraid_raid_common_gf8_log[256];

/* GF(2^8) 反对数表（α^x, x=0..254; 扩展到 512 以避免 mod 255）*/
extern uint8_t poweraid_raid_common_gf8_exp[512];

/* ===== 内部: 标量运算（供软件参考实现和测试用）===== */

static inline uint8_t
poweraid_raid_common_gf8_mul_scalar(uint8_t a, uint8_t b)
{
	if (a == 0 || b == 0) {
		return 0;
	}
	return poweraid_raid_common_gf8_exp[poweraid_raid_common_gf8_log[a] + poweraid_raid_common_gf8_log[b]];
}

static inline uint8_t
poweraid_raid_common_gf8_div_scalar(uint8_t a, uint8_t b)
{
	if (a == 0) {
		return 0;
	}
	/* b != 0 前提（调用方保证）*/
	return poweraid_raid_common_gf8_exp[(uint32_t)(poweraid_raid_common_gf8_log[a] + 255 - poweraid_raid_common_gf8_log[b]) % 255];
}

static inline uint8_t
poweraid_raid_common_gf8_inv_scalar(uint8_t a)
{
	/* a != 0 前提 */
	return poweraid_raid_common_gf8_exp[255 - poweraid_raid_common_gf8_log[a]];
}
 ...
 *   - dst 可 == src（原地乘法）
 */
typedef void (*poweraid_raid_common_gf8_mul_const_fn)(const uint8_t *src, uint8_t constant,
		uint8_t *dst, uint64_t len);

/* gf8_mul_const_xor: dst[i] ^= constant × src[i]（累加模式，用于 encode）*/
typedef void (*poweraid_raid_common_gf8_mul_const_xor_fn)(const uint8_t *src, uint8_t constant,
		uint8_t *dst, uint64_t len);

/* 获取当前激活的 SIMD 路径名（调试/日志用）*/
const char *poweraid_raid_common_gf8_impl_name(void);
 ...
 *   p_impl_name:     实现名（调试用）
 */
void poweraid_raid_common_gf8_try_select_simd(
...
#endif /* POWERAID_RAID_COMMON_GF8_H */
