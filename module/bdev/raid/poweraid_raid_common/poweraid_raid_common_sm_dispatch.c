 ...
 *
 *   三层 FSM 通用分派入口：poweraid_raid_common_sm_process()
 *
 *   分派表本体在各自的 .c 文件中定义（poweraid_raid_common_sm_raid.c /
 *   poweraid_raid_common_sm_bdev.c / poweraid_raid_common_sm_req.c），
...
void
poweraid_raid_common_sm_process(enum poweraid_raid_common_fsm_layer layer, void *obj,
			  uint32_t event)
{
	...
	case POWERAID_FSM_LAYER_RAID:
		if (event < POWERAID_RAID_EV_COUNT_REAL &&
		    poweraid_raid_common_raid_fsm[event] != NULL) {
			poweraid_raid_common_raid_fsm[event](
				(struct poweraid_raid_common_raid *)obj,
	...
	case POWERAID_FSM_LAYER_BDEV:
		if (event < POWERAID_BDEV_EV_COUNT &&
		    poweraid_raid_common_bdev_fsm[event] != NULL) {
			poweraid_raid_common_bdev_fsm[event](
				(struct poweraid_raid_common_bdev *)obj,
	...
	case POWERAID_FSM_LAYER_REQ:
		if (event < POWERAID_REQ_EV_COUNT &&
		    poweraid_raid_common_req_fsm[event] != NULL) {
			poweraid_raid_common_req_fsm[event](
				(struct poweraid_raid_common_req *)obj,
