...
#ifndef POWERAID_RAID5F_PPL_H
#define POWERAID_RAID5F_PPL_H
...
#define POWERAID_RAID5F_PPL_REC_MAGIC    0x504C5052u  /* "PPLR" little-endian */
#define POWERAID_RAID5F_PPL_SUPER_MAGIC  0x50505352u  /* "PPSR" little-endian */

/* PPL super 大小（与 record 一致，64B）*/
#define POWERAID_RAID5F_PPL_SUPER_SIZE   64
/* PPL record 大小 */
#define POWERAID_RAID5F_PPL_REC_SIZE     64

/* 默认 PPL 区几何（与 sb v2 ext 默认值一致，可被 sb ext 覆盖）*/
#define POWERAID_RAID5F_PPL_DEFAULT_REGION_OFFSET  4096ULL          /* 4 KiB */
#define POWERAID_RAID5F_PPL_DEFAULT_REGION_SIZE    (4ULL * 1024 * 1024)  /* 4 MiB */

/* record flags */
#define POWERAID_RAID5F_PPL_REC_F_VALID     (1u << 0)  /* record 有效（magic+crc 通过）*/
#define POWERAID_RAID5F_PPL_REC_F_COMMITTED (1u << 1)  /* data+parity 已写并 flush（用于 in-place 标记，阶段 2 用 super.commit_seq 判定，此 flag 备用）*/
 ...
 */
struct poweraid_raid5f_ppl_record {
	uint32_t	magic;            /* POWERAID_RAID5F_PPL_REC_MAGIC */
	uint32_t	flags;             /* POWERAID_RAID5F_PPL_REC_F_* */
...
SPDK_STATIC_ASSERT(sizeof(struct poweraid_raid5f_ppl_record) ==
		   POWERAID_RAID5F_PPL_REC_SIZE, "incorrect ppl record size");

/*
 * PPL super（64B），位于 PPL 区起始。
 * head_seq / tail_seq / commit_seq 均为 record seq（非槽位下标）。
 */
struct poweraid_raid5f_ppl_super {
	uint32_t	magic;            /* POWERAID_RAID5F_PPL_SUPER_MAGIC */
	uint32_t	version;          /* 1 */
...
SPDK_STATIC_ASSERT(sizeof(struct poweraid_raid5f_ppl_super) ==
		   POWERAID_RAID5F_PPL_SUPER_SIZE, "incorrect ppl super size");

/* ===== 上下文（opaque）===== */
struct poweraid_raid5f_ppl_ctx;

/* in-flight record（已 append，待 commit），挂在 ctx->inflight 链表 */
struct poweraid_raid5f_ppl_inflight {
	uint64_t				seq;
	uint64_t				stripe_id;
	TAILQ_ENTRY(poweraid_raid5f_ppl_inflight)	link;
};

/* ===== 回调原型 ===== */
typedef void (*poweraid_raid5f_ppl_init_cb)(int status, void *cb_arg);
typedef void (*poweraid_raid5f_ppl_append_cb)(int status, uint64_t seq, void *cb_arg);
typedef void (*poweraid_raid5f_ppl_commit_cb)(int status, void *cb_arg);

/*
 * recovery 回调：load_replay 完成后返回按 seq 升序排列的未 commit record 数组。
 * 调用方负责 free(records)（用 poweraid_raid5f_ppl_free_replay_result）。
 * status==0 且 records!=NULL 时 num_records>0；status!=0 时 records 为 NULL。
 */
typedef void (*poweraid_raid5f_ppl_replay_cb)(int status,
 ...
 */
struct poweraid_raid5f_ppl_ctx *poweraid_raid5f_ppl_alloc(
 ...
 */
void poweraid_raid5f_ppl_free(struct poweraid_raid5f_ppl_ctx *ctx);

/**
 * 初始化磁盘 PPL 区：写 super（head=tail=commit=next_seq=1）+ FUA。
 * 用于新卷创建或首次格式化。异步。
 */
void poweraid_raid5f_ppl_init(struct poweraid_raid5f_ppl_ctx *ctx,
 ...
 * 对应写路径第 1 步（MD L273）。
 */
void poweraid_raid5f_ppl_append_record(
	struct poweraid_raid5f_ppl_ctx *ctx, struct spdk_io_channel *ch,
 ...
 */
void poweraid_raid5f_ppl_commit(struct poweraid_raid5f_ppl_ctx *ctx,
 ...
 * 子任务 C 串联调用。
 */
void poweraid_raid5f_ppl_load_replay(struct poweraid_raid5f_ppl_ctx *ctx,
				     poweraid_raid5f_ppl_replay_cb cb, void *cb_arg);

/**
 * 释放 load_replay 回调返回的 record 数组。
 */
void poweraid_raid5f_ppl_free_replay_result(struct poweraid_raid5f_ppl_record *records);
 ...
 */
uint64_t poweraid_raid5f_ppl_data_hash(const void *buf, size_t len);
 ...
 */
struct poweraid_raid5f_ppl_hash_ctx {
	uint32_t	lo;
	uint32_t	hi;
};
void poweraid_raid5f_ppl_hash_init(struct poweraid_raid5f_ppl_hash_ctx *c);
void poweraid_raid5f_ppl_hash_update(struct poweraid_raid5f_ppl_hash_ctx *c,
				     const void *buf, size_t len);
uint64_t poweraid_raid5f_ppl_hash_final(struct poweraid_raid5f_ppl_hash_ctx *c);

#ifdef __cplusplus
}
#endif

#endif /* POWERAID_RAID5F_PPL_H */
